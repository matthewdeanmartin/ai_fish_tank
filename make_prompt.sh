...
# readonly SCRIPT_NS="ai_fish_tank_proc"
readonly SRC_DIR="./ai_fish_tank"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/ai_fish_tank"

# Copy directory safely
ai_fish_tank_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/ai_fish_tank
ai_fish_tank_proc::run_strip_docs() {
  local dir
...
ai_fish_tank_proc::flatten_repo() {
...
main() {
  ai_fish_tank_proc::copy_to_tmp
  ai_fish_tank_proc::run_strip_docs
  ai_fish_tank_proc::confirm "Run coderoller-flatten-repo?"
  ai_fish_tank_proc::flatten_repo
  ai_fish_tank_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  ai_fish_tank_proc::cleanup_tmp
}
